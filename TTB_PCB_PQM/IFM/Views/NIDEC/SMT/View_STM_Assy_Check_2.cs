...
namespace IFM.Views.NIDEC.SMT
{
    public partial class View_STM_Assy_Check_2 : RibbonForm
    {
        ...
        bool checkcondition()
        {
            if (txt_barcode.Text.Length < 5 || txt_pcbbarcode.Text.Length < 5 || cbm_modelcd.Text == "")
            {
            ...
            string result_ = result(cbm_modelcd.Text + DateTime.Now.ToString("yyyyMMdd"));
            if (result_ == "")
                result_ = result(cbm_modelcd.Text + "202306");
            if (result_ == "")
            {
