...
{
    public partial class View_STM_Mounter_Master : RibbonForm
    {
        private readonly DkGridView<m_smt_mounter> _gridData;
        private readonly SMTMounter_GetMasterQuery _refreshQuery = new SMTMounter_GetMasterQuery();


        public View_STM_Mounter_Master()
        {
            InitializeComponent();
            _gridData = new UserRolesGridViewT(gv_data);
            _gridData.AfterAddRow += (obj, e) =>
            {
                var item = gv_data.GetRow(e) as m_smt_mounter;
        ...
        private void Gv_data_CustomDrawCell(object sender, DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs e)
        {
            try
            {
                var item = gv_data.GetRow(e.RowHandle) as m_smt_mounter;
                if (item !=null)
                {
        ...
        private void BbiSave_ItemClick(object sender, ItemClickEventArgs e)
        {
            try
            {
                ...
                {
                    _gridData.UpdateDB(new SMTMounter_UpdateMasterCommand(_gridData.LstModifired.ToArray()));
                }
        ...
        private void gv_data_PopupMenuShowing(object sender, PopupMenuShowingEventArgs e)
        {
            ...
            OnGrid_CQRS.command_ = new SMTMounter_DeleteMasterCommand(OnGrid_CQRS.ID_);
            OnGrid_CQRS CallDeleteRow = new OnGrid_CQRS();
            CallDeleteRow.MouseRight(sender, e);
            _gridData.RefreshData(_refreshQuery);
        }
        internal class UserRolesGridViewT : DkGridView<m_smt_mounter>
        {
