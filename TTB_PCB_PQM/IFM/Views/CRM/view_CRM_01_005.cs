...
{
    public partial class view_CRM_01_005 : Form
    {

        public view_CRM_01_005()
        {
            InitializeComponent();
        }
        DataTable dt;
        ...
        readonly string sqldelete = "delete from t_pay_money where 1=1  and ";
        readonly string sqlproject = "select distinct(project_name) from m_project order by project_name";


        private void view_CRM_01_005_Load(object sender, EventArgs e)
        {
            GetCombox();
            LoadData(false);
            GetLessMoneyValue();
        ...
        private void rdb_checking_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                gv_main.Columns[8].OptionsColumn.ReadOnly = false;
            }
            else
            {
                gv_main.Columns[8].OptionsColumn.ReadOnly = true;
            }
        }
        ...
        void LoadData(bool filterfull)
        {
            try
            {
                ...
                sqlload.Append("select * from t_pay_money where 1=1 ");
        ...
        void GetLessMoneyValue()
        {
            Class.mysqlconnection con = new Class.mysqlconnection();
            string sqlGetLessMoneyValue = @"select  ifnull(max(less_money_value),0) as less_money_value
                                    from t_pay_money WHERE d_get =
                                    (select max(d_get) as maxday  from t_pay_money)";
            lbl_lessmoneyvalue.Text ="Quỹ Tiền Mặt Còn Lại:  "+ string.Format("{0:#,##0.00}", double.Parse(con.sqlExecuteScalarString(sqlGetLessMoneyValue).Trim()))  + "   VND";
        }
        ...
        bool checkcondition()
        {
            if (cbm_username_get.SelectedItem == null || cbm_user_app.SelectedItem == null || cbm_type_content.SelectedItem == null || cbm_project_name.SelectedItem == null || calc_money.Text == "" || calc_money.Value ==0)
            {
                MessageBox.Show("Chưa chọn đầy đủ Thông Tin", "Thông Báo Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            if (txt_contents.Text.Length < 15)
            {
        ...
        {
            dt = new DataTable();
            string sql = @"select ifnull(max(money_value),0) as money_value , ifnull(max(less_money_value),0) as less_money_value
                        from t_pay_money WHERE d_get =
                        (select max(d_get) as maxday  from t_pay_money)  ";
            Class.mysqlconnection con = new Class.mysqlconnection();
            con.sqlDataAdapterFillDatatable(sql, ref dt);
            //float money_value_old = float.Parse(dt.Rows[0]["money_value"].ToString().Trim());
            float less_money_value = float.Parse(dt.Rows[0]["less_money_value"].ToString().Trim());
            ...
            try
            {
                if (checkcondition())
                {
                    ...
                    row["content"] = txt_contents.Text;
                    row["contentdetails"] = txt_contentdetails.Text;
                    if(rdb_chi.Checked ==true)                      
                    {
                        row["money_value"] = calc_money.Value;
                        row["less_money_value"] = less_money_value - float.Parse(calc_money.Text);
                        thu_chi = "Chi Ra";
                    }   
                    else
                    {
                        row["money_value"] = 0;
                        row["less_money_value"] = float.Parse(calc_money.Text) + less_money_value;
                        thu_chi = "Thu Vào";
                    }    
        ...
        void email()
        {
            ...
            //String SendMailTo = "long.thanhle@ifmsolutionsvn.com";
            String ccMail = "info@ifmsolutionsvn.com";
            String SendMailSubject = "PayGetMoney ";
            string htmlString = @"Dear All  
           
            Bạn " + ClsSession.App.UserName + @" Vừa Thực Hiện Thao Tác " + thu_chi + @" với Nội Dung như Sau:
            --------  
            1. Số Tiền "+thu_chi+": " + string.Format("{0:#,##0.00}", double.Parse(calc_money.Text)) + @"   VND;
            2. "+ thu_chi + " Cho Ai: " + cbm_username_get.Text + @"
            3. Tên Dự Án:  " + cbm_project_name.Text + @"
            4. Nội Dung "+thu_chi+":  " + txt_contents.Text + @"
            5. Chi Tiết <nếu có>:  " + txt_contentdetails.Text + @"
            6. Người Duyệt:  " + cbm_user_app.Text + @"
            7. Ngày Giờ "+thu_chi+":  " + DateTime.Now.ToString() + @"
            ***
            Số Tiền Quỹ Còn Lại <Nếu có> : " + lbl_lessmoneyvalue.Text + @"
        ...
        void savedb()
        {
            ...
            foreach (DataRow rowdt in dt.Rows)
            {
                if (rowdt["ID"].ToString() == "")
                {
                    ...
                    sqlinsert.Append(@"INSERT INTO t_pay_money
                                    (content,contentdetails, money_value, less_money_value, u_get, u_pay, type_content, project_name, d_get, status, 
                                        user_name_App, u_update, d_update, u_create, d_create)
                                    VALUES("
                                    );
                    sqlinsert.Append("'" + rowdt["content"].ToString() + "',");
                    sqlinsert.Append("'" + rowdt["contentdetails"].ToString() + "',");
                    sqlinsert.Append("'" + rowdt["money_value"].ToString() + "',");
                    sqlinsert.Append("'" + rowdt["less_money_value"].ToString() + "',");
                    ...
                    sqlinsert.Append("'" + function.ConvertDateFull(rowdt["d_get"].ToString()) + "',");
                ...
                else
                {
                    ...
                    sqlupdate.Append(@"UPDATE t_pay_money set ");
                    sqlupdate.Append("content = '" + rowdt["content"].ToString() + "',");
                    sqlupdate.Append("contentdetails = '" + rowdt["contentdetails"].ToString() + "',");
                    sqlupdate.Append("money_value = '" + rowdt["money_value"].ToString() + "',");
                    sqlupdate.Append("less_money_value = '" + rowdt["less_money_value"].ToString() + "',");
